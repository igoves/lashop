...
@section('content')
<div>
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Page: {{ $page->title }}</h1>
        ...
        <form action="{{ route('admin.pages.update', $page) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="show_in_menu" name="show_in_menu[]" multiple
                    ...
                    <option value="menu" {{ old('show_in_menu', $page->show_in_menu ? ['menu'] : []) == 'menu' || (is_array(old('show_in_menu', [])) && in_array('menu', old('show_in_menu', []))) || $page->show_in_menu ? 'selected' : '' }}>Menu</option>
                    <option value="footer" {{ old('show_in_footer', $page->show_in_footer ? ['footer'] : []) == 'footer' || (is_array(old('show_in_footer', [])) && in_array('footer', old('show_in_footer', []))) || $page->show_in_footer ? 'selected' : '' }}>Footer</option>
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea id="fulldesc" name="fulldesc" rows="10"
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm @error('fulldesc') border-red-500 @enderror">{{ old('fulldesc', $page->fulldesc) }}</textarea>
                @error('fulldesc')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
