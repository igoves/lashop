...
<div>
    ...
    <form method="GET" class="mb-4 flex gap-2">
        ...
        @if(!empty($search))
            <a href="{{ route('admin.pages.index') }}" class="px-4 py-2 text-gray-500 text-sm hover:underline">Clear</a>
        @endif
    </form>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($pages as $page)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $page->title }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-700">{{ $page->slug }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if($page->show_in_menu && $page->show_in_footer)
                                <span class="px-2 py-0.5 text-xs rounded bg-blue-100 text-blue-700">Menu</span>
                                <span class="px-2 py-0.5 text-xs rounded bg-purple-100 text-purple-700">Footer</span>
                            @elseif($page->show_in_menu)
                                <span class="px-2 py-0.5 text-xs rounded bg-blue-100 text-blue-700">Menu</span>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-sm text-gray-500 text-center">No pages found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
