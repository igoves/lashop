        ...
        <form action="{{ route('admin.news.update', $news) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea id="fulldesc" name="fulldesc" rows="10"
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm @error('fulldesc') border-red-500 @enderror">{{ old('fulldesc', $news->fulldesc) }}</textarea>
                @error('fulldesc')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="meta_description" name="meta_description" value="{{ old('meta_description', $news->meta_description) }}"
