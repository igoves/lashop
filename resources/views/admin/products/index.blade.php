...
<div>
    ...
    <form method="GET" class="mb-4 flex flex-wrap gap-2 items-center">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name..."
               class="flex-1 min-w-[200px] max-w-xs rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
        <select name="brand_id" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
            <option value="">All brands</option>
            @foreach($brands as $id => $title)
                <option value="{{ $id }}" {{ request('brand_id') == $id ? 'selected' : '' }}>{{ $title }}</option>
            @endforeach
        </select>
        <select name="category_id" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
            ...
            @foreach($categories as $id => $title)
                <option value="{{ $id }}" {{ request('category_id') == $id ? 'selected' : '' }}>{{ $title }}</option>
            @endforeach
        </select>
        ...
        @if(request('search') || request('brand_id') || request('category_id'))
            <a href="{{ route('admin.products.index') }}" class="px-4 py-2 text-gray-500 text-sm hover:underline">Clear</a>
        @endif
    </form>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($products as $product)
                    <tr>
                        <td class="px-4 py-3">
                            @if($product->photo)
                                <img src="{{ asset('uploads/products/small/' . $product->photo) }}"
                                     alt="{{ $product->title }}"
                        ...
                        <td class="px-4 py-3 text-sm text-gray-900 font-medium">{{ $product->title }}</td>
                        <td class="px-4 py-3 text-sm text-gray-500 font-mono">{{ $product->slug }}</td>
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $product->category?->title }}</td>
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $product->brand?->title }}</td>
                        <td class="px-4 py-3 text-sm text-gray-900">{{ number_format($product->cost, 2) }}</td>
                        <td class="px-4 py-3 text-sm">
                            @if($product->isActive())
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Active</span>
                            @else
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-600">Hidden</span>
                            @endif
                        </td>
