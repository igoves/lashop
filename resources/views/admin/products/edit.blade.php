        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select id="cat_id" name="cat_id"
                    ...
                    @foreach($categories as $id => $title)
                        <option value="{{ $id }}" {{ old('cat_id', $product->cat_id) == $id ? 'selected' : '' }}>{{ $title }}</option>
                    @endforeach
                </select>
                @error('cat_id')
            ...
            <div class="mb-4">
                ...
                <select id="brand_id" name="brand_id"
                    ...
                    @foreach($brands as $id => $title)
                        <option value="{{ $id }}" {{ old('brand_id', $product->brand_id) == $id ? 'selected' : '' }}>{{ $title }}</option>
                    @endforeach
                </select>
                @error('brand_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <textarea id="fulldesc" name="fulldesc" rows="6"
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm @error('fulldesc') border-red-500 @enderror">{{ old('fulldesc', $product->fulldesc) }}</textarea>
                @error('fulldesc')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
                ...
                @if($product->photo)
                    <div class="mb-2">
                        <img src="{{ asset('uploads/products/small/' . $product->photo) }}"
                             alt="{{ $product->title }}"
                             class="w-24 h-18 object-cover rounded border border-gray-200">
                        <p class="text-xs text-gray-500 mt-1">Current photo. Upload a new one to replace it.</p>
                    </div>
                @endif
